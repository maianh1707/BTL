...
namespace baitaplon.Forms
{
    public partial class baocaotonkho : Form
    {
        ...
        {
            string sql;
            sql = "SELECT a.masanpham, b.tensanpham, a.soluongnhap, a.soluongxuat, a.soluongtonkho, a.soluongtonkhothucte, a.soluongchenhlech " +
            ...
            dgridkiemke.Columns[2].HeaderText = "Số lượng nhập";
            dgridkiemke.Columns[3].HeaderText = "Số lượng xuất";
            dgridkiemke.Columns[4].HeaderText = "Số lượng tồn kho";
            dgridkiemke.Columns[5].HeaderText = "Số lượng tồn kho thực tế";
            dgridkiemke.Columns[6].HeaderText = "Số lượng chênh lệch";
            ...
            dgridkiemke.Columns[4].Width = 120;
            dgridkiemke.Columns[5].Width = 120;
        ...
        private void btnluu_Click(object sender, EventArgs e)
        {
            ...
            // Kiểm tra nếu mã kiểm kê đã tồn tại
            sql = "SELECT makiemke FROM tblbckiemkho WHERE makiemke=N'" + txtmakiemke.Text + "'";
            if (!Functions.CheckKey(sql))
            {
                // Mã kiểm kê chưa có, tiến hành lưu các thông tin chung
                ...
                // Lưu thông tin chung vào bảng tbldonkiemke    
            ...
            //if (txtsoluongxuat.Text.Trim().Length == 0)
            //{
            ...
            sql = "SELECT masanpham FROM tblchitietbckiemkho WHERE masanpham=N'" +
                  cbomasp.SelectedValue + "' AND makiemke = N'" + txtmakiemke.Text.Trim() + "'";
            if (Functions.CheckKey(sql))
            {
            ...
            // Lưu thông tin chi tiết vào bảng tblchitietdonkiemke
            sql = "INSERT INTO tblchitietbckiemkho(makiemke, masanpham, soluongxuat, soluongnhap, soluongtonkho, soluongtonkhothucte, soluongchenhlech) " +
                  "VALUES(N'" + txtmakiemke.Text.Trim() + "', N'" + cbomasp.SelectedValue + "', " +
                  txtsoluongxuat.Text + ", " + txtsoluongnhap.Text + ", " + soluongtonkho + ", " +
                  soluongtonkhothucte + ", " + chenhlech + ")";
            Functions.RunSql(sql);
        ...
        private void cbomasp_TextChanged(object sender, EventArgs e)
        {
            ...
            // Lệnh SQL để lấy tên hàng, số lượng nhập và số lượng xuất
            str = @"SELECT sp.tensanpham,
                            COALESCE(SUM(cthdn.soluongnhap), 0) AS soluongnhap,
                            COALESCE(SUM(cthdb.soluongxuat), 0) AS soluongxuat
                    FROM tblsanpham sp
                        LEFT JOIN tblchitiethdn cthdn ON sp.masanpham = cthdn.masanpham
                        LEFT JOIN tblchitiethdb cthdb ON sp.masanpham = cthdb.masanpham
                    WHERE sp.masanpham = N'" + cbomasp.SelectedValue + @"'
                    GROUP BY sp.tensanpham";

            // Sử dụng phương thức GetFieldValues để lấy dữ liệu từ truy vấn
        ...
        private void btninkiemke_Click(object sender, EventArgs e)
        {
            ...
            exRange.Range["C2:E2"].Value = "PHIẾU KIỂM KHO";
            ...
            sql = "SELECT masanpham, soluongnhap, soluongxuat, soluongtonkho, soluongtonkhothucte, soluongchenhlech FROM tblchitietbckiemkho WHERE makiemke = N'" + txtmakiemke.Text + "'";
            tblThongtinHang = Functions.GetDataToTable(sql);

            // Tạo dòng tiêu đề bảng
            exRange.Range["A8:G8"].Font.Bold = true;
            exRange.Range["C8:G8"].HorizontalAlignment = COMExcel.XlHAlign.xlHAlignCenter;
            exRange.Range["A8:A8"].Value = "STT";
            exRange.Range["B8:B8"].Value = "Mã sản phẩm";
            exRange.Range["C8:C8"].Value = "Số lượng nhập";
            exRange.Range["D8:D8"].Value = "Số lượng xuất";
            exRange.Range["E8:E8"].Value = "Số lượng tồn kho";
            exRange.Range["F8:F8"].Value = "Số lượng thực tế";
            exRange.Range["G8:G8"].Value = "Số lượng chênh lệch";

            // Kéo dài ô để hiển thị hết nội dung tiêu đề
            exRange.Range["A8:A8"].Columns.AutoFit();
            exRange.Range["C8:G8"].Columns.AutoFit();
            ...
            exRange = exSheet.Range["A8:G" + (hang + 8).ToString()];
